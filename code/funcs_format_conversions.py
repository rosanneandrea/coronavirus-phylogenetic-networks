...
def semitemp_extract_tcs(infilename):
	infile = open(f"{resultspath}temporal/rerun/{infilename}", "r")
	outfilename = infilename.replace(".log", ".seq")
	
	sequence = False
	for line in infile.read().split("\n"):
		if " -> " in line:
			sequence = True
			tcs = line.strip().strip(" ->")
	
	if sequence == True:
		with open(f"{resultspath}temporal/rerun/{outfilename}", "w") as outfile:
			outfile.write(tcs)
